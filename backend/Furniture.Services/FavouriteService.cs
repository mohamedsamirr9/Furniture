...
namespace Furniture.Services
{
    public class FavouriteService : IFavouriteService
    {
        ...
        public async Task<FavouriteDto> AddToFavouritesAsync(string userId, int productId)
        {
            ...
            var existing = (await _unitOfWork.GetRepository<Favourite, int>().GetAllAsync(existingSpec)).FirstOrDefault();

            if (existing != null) throw new InvalidOperationException("Product Is Already In Favourites");
